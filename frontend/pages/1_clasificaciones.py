...
if not bracket:
    ...
    bracket_cols = st.columns(len(STAGE_LABELS))

    for col, (stage_key, stage_label) in zip(bracket_cols, STAGE_LABELS.items()):
        with col:
            ...
            for m in matches:
                ...
                home_display = f"**{home}**" if home_winner else home
                away_display = f"**{away}**" if away_winner else away

                h_str = str(h_score) if h_score is not None else "-"
                a_str = str(a_score) if a_score is not None else "-"

                with st.container(border=True):
                    st.markdown(f"{home_display} — {h_str}")
                    st.markdown(f"{away_display} — {a_str}")

    st.caption("🟢 En negrita = equipo que avanzó · TBD = por determinar")
